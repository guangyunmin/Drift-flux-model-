...
% Naming note: legacy variable and output-file identifiers containing PIE_DF
% are retained for backward compatibility. Paper-facing labels are standardized
% as PGE-DFM (expert ensemble) and PGE-DFM-RCN (residual correction).
clc;
clearvars;
...
for k = 1:length(counts)
    ...
    for ii = 1:length(idx_range)
        ...
        Re_gi = Re_g(i);
        Rei = max(Re_fi, Re_gi);
        % Target-independent initial guess.  Never initialize an expert with
        % the measured void fraction, because that leaks the response into
        % the predictor and can select a different fixed-point branch.
        alpha_old = initialAlphaFromFluxPIEOneClick(jgi, jtoti);
        alpha_new = NaN;
        for iter = 1:max_iter
            % ==== 模型参数 ====
...
for k = 1:length(counts)
    ...
    for i = 1:N_seg
        jg_i = jg_seg(i);
        jt_i = jt_seg(i);
        % Homogeneous void fraction from superficial velocities only.
        alpha_old = initialAlphaFromFluxPIEOneClick(jg_i, jt_i);
        alpha_new = NaN;
...
for k = 1:length(counts)
    ...
    for i = 1:N_seg
        alpha_old = initialAlphaFromFluxPIEOneClick(jg_seg(i), jt_seg(i));
        alpha_new = NaN;
...
%% ========================================================================
%  6. 构造监督权重 Wtrue（tau 仅由训练标签估计）
%
%  重要：验证集和测试集标签不得参与任何训练超参数或软标签的构造。
% ========================================================================

absErr = abs(A_valid - Y_valid);
tau = median(absErr(idxTrain,:), 'all', 'omitnan');
...
Wtrue = Wtrue ./ max(sum(Wtrue, 2), eps);
...
for mm = 1:M
    fprintf('%-22s : %.2f %%\n', modelNames{mm}, ...
        100*mean(bestModelEachPoint(idxTrain) == mm));
end

%% ========================================================================
%  7. 标准化输入（均值与标准差仅由训练集估计）
% ========================================================================
...
% Matched purely data-driven baseline requested by the reviewer.  It uses
% exactly the same 43 normalized inputs and the same fixed split as PGE-DFM,
% but it predicts void fraction directly and never receives DFM predictions.
% Its 108-80-40 architecture has 16,753 trainable parameters, slightly more
% than PGE-DFM plus its residual-correction network (16,386 parameters).
layersDirectTemplate = [
    featureInputLayer(numFeatures, "Normalization","none", "Name","input")
    fullyConnectedLayer(108, "Name","fc1")
    tanhLayer("Name","tanh1")
    dropoutLayer(0.05, "Name","drop1")
    ...
    tanhLayer("Name","tanh3")
    fullyConnectedLayer(1, "Name","alpha_logit")
...
% ------------------------------------------------------------------------
% 逐 seed 结果表。EqualWeight 和 StaticWeight 是权重消融；DirectMLP 是
% 同输入、同划分、同训练种子的纯数据驱动基线。
% ------------------------------------------------------------------------
seedResults = table(trainingSeeds(:), 'VariableNames', {'Seed'});
metricNames = {'ARE','MAE','RMSE','R2'};
modelPrefixes = {'PGE_DFM','PGE_DFM_RCN','DirectMLP','EqualWeight','StaticWeight'};
for iModelPrefix = 1:numel(modelPrefixes)
    for iMetric = 1:numel(metricNames)
        seedResults.([modelPrefixes{iModelPrefix} '_Test_' metricNames{iMetric}]) = NaN(nSeeds,1);
    end
end
seedResults.PGE_DFM_ARE_Improvement_vs_BestTraditional_percent = NaN(nSeeds,1);
seedResults.PGE_DFM_RCN_ARE_Improvement_vs_BestTraditional_percent = NaN(nSeeds,1);
seedResults.PGE_DFM_ARE_Improvement_vs_DirectMLP_percent = NaN(nSeeds,1);
seedResults.PGE_DFM_ARE_Improvement_vs_StaticWeight_percent = NaN(nSeeds,1);
...
for iSeed = 1:nSeeds
    ...
    for epoch = 1:numEpochs
        ...
        for start = 1:miniBatchSize:numel(idxTrainShuffle)
            ...
            epochLossSum = epochLossSum + double(gather(extractdata(loss))) * batchCount;
            epochSampleCount = epochSampleCount + batchCount;
        ...
        historyExpert(epoch,:) = [epochLossSum / max(epochSampleCount,1), ...
    ...
    for epoch = 1:numEpochsErr
        ...
        for start = 1:miniBatchSizeErr:numel(idxTrainShuffle)
            ...
            batchCount = numel(batchIdx);
            epochLossSum = epochLossSum + double(gather(extractdata(lossErr))) * batchCount;
            epochSampleCount = epochSampleCount + batchCount;
        ...
        historyErr(epoch,:) = [epochLossSum / max(epochSampleCount,1), ...
    ...
    %% --------------------------------------------------------------------
    %  10B. 训练匹配的纯数据驱动 Direct-MLP 基线
    % ---------------------------------------------------------------------
    % Resetting to the same seed makes the stochastic protocol explicit and
    % independent of how many random numbers the two PGE stages consumed.
    rng(thisSeed, 'twister');
    netDirect = dlnetwork(layerGraph(layersDirectTemplate));
    learnRateDirect = learnRateDirect0;
    avgGradDirect = [];
    avgSqGradDirect = [];
    iterationDirect = 0;
    historyDirect = zeros(numEpochsDirect, 3);

    for epoch = 1:numEpochsDirect
        idxTrainShuffle = idxTrain(randperm(numel(idxTrain)));
        epochLossSum = 0;
        epochSampleCount = 0;

        for start = 1:miniBatchSize:numel(idxTrainShuffle)
            iterationDirect = iterationDirect + 1;
            batchIdx = idxTrainShuffle(start:min(start+miniBatchSize-1, numel(idxTrainShuffle)));

            dlX = dlarray(single(X_valid(batchIdx,:)'), 'CB');
            dlY = dlarray(single(Y_valid(batchIdx)'), 'CB');
            [lossDirect, gradientsDirect] = dlfeval(@directGradientsPIEOneClick, ...
                netDirect, dlX, dlY);

            batchCount = numel(batchIdx);
            epochLossSum = epochLossSum + double(gather(extractdata(lossDirect))) * batchCount;
            epochSampleCount = epochSampleCount + batchCount;

            [netDirect, avgGradDirect, avgSqGradDirect] = adamupdate( ...
                netDirect, gradientsDirect, avgGradDirect, avgSqGradDirect, ...
                iterationDirect, learnRateDirect, gradDecay, sqGradDecay);
        end

        if mod(epoch, 300) == 0
            learnRateDirect = learnRateDirect * 0.5;
        end

        alphaValDirect = predictDirectPIEOneClick(netDirect, X_valid(idxVal,:));
        metValDirect = calcMetricsPIEOneClick_R2FIX20260513(Y_valid(idxVal), alphaValDirect);
        historyDirect(epoch,:) = [epochLossSum / max(epochSampleCount,1), ...
                                  metValDirect.ARE, metValDirect.RMSE];

        if mod(epoch, 100) == 0 || epoch == 1 || epoch == numEpochsDirect
            fprintf('[Seed %d][Direct-MLP] Epoch %4d/%4d | Val ARE %.4f | Val RMSE %.4f\n', ...
                thisSeed, epoch, numEpochsDirect, metValDirect.ARE, metValDirect.RMSE);
        end
    end

    alphaTrainDirect = predictDirectPIEOneClick(netDirect, X_valid(idxTrain,:));
    alphaValDirect   = predictDirectPIEOneClick(netDirect, X_valid(idxVal,:));
    alphaTestDirect  = predictDirectPIEOneClick(netDirect, X_valid(idxTest,:));
    metDirectTrain = calcMetricsPIEOneClick_R2FIX20260513(Y_valid(idxTrain), alphaTrainDirect);
    metDirectVal   = calcMetricsPIEOneClick_R2FIX20260513(Y_valid(idxVal), alphaValDirect);
    metDirectTest  = calcMetricsPIEOneClick_R2FIX20260513(Y_valid(idxTest), alphaTestDirect);

    %% --------------------------------------------------------------------
    %  10C. Equal-weight and fixed-weight ablations
    % ---------------------------------------------------------------------
    alphaTestEqual = clipAlphaPIEOneClick(mean(A_valid(idxTest,:), 2));
    metEqualTest = calcMetricsPIEOneClick_R2FIX20260513(Y_valid(idxTest), alphaTestEqual);

    WtrainLearned = predictWeightsPIEOneClick(netExpert, X_valid(idxTrain,:));
    wStaticTrain = mean(WtrainLearned, 1, 'omitnan');
    wStaticTrain = wStaticTrain ./ max(sum(wStaticTrain), eps);
    alphaTestStatic = clipAlphaPIEOneClick(A_valid(idxTest,:) * wStaticTrain(:));
    metStaticTest = calcMetricsPIEOneClick_R2FIX20260513(Y_valid(idxTest), alphaTestStatic);

    %% --------------------------------------------------------------------
    %  记录该 seed 的测试集结果
    % ---------------------------------------------------------------------
    seedResults.PGE_DFM_Test_ARE(iSeed)  = metPIETest.ARE;
    seedResults.PGE_DFM_Test_MAE(iSeed)  = metPIETest.MAE;
    seedResults.PGE_DFM_Test_RMSE(iSeed) = metPIETest.RMSE;
    seedResults.PGE_DFM_Test_R2(iSeed)   = metPIETest.R2;

    seedResults.PGE_DFM_RCN_Test_ARE(iSeed)  = metPIEETest.ARE;
    seedResults.PGE_DFM_RCN_Test_MAE(iSeed)  = metPIEETest.MAE;
    seedResults.PGE_DFM_RCN_Test_RMSE(iSeed) = metPIEETest.RMSE;
    seedResults.PGE_DFM_RCN_Test_R2(iSeed)   = metPIEETest.R2;

    seedResults.DirectMLP_Test_ARE(iSeed)  = metDirectTest.ARE;
    seedResults.DirectMLP_Test_MAE(iSeed)  = metDirectTest.MAE;
    seedResults.DirectMLP_Test_RMSE(iSeed) = metDirectTest.RMSE;
    seedResults.DirectMLP_Test_R2(iSeed)   = metDirectTest.R2;

    seedResults.EqualWeight_Test_ARE(iSeed)  = metEqualTest.ARE;
    seedResults.EqualWeight_Test_MAE(iSeed)  = metEqualTest.MAE;
    seedResults.EqualWeight_Test_RMSE(iSeed) = metEqualTest.RMSE;
    seedResults.EqualWeight_Test_R2(iSeed)   = metEqualTest.R2;

    seedResults.StaticWeight_Test_ARE(iSeed)  = metStaticTest.ARE;
    seedResults.StaticWeight_Test_MAE(iSeed)  = metStaticTest.MAE;
    seedResults.StaticWeight_Test_RMSE(iSeed) = metStaticTest.RMSE;
    seedResults.StaticWeight_Test_R2(iSeed)   = metStaticTest.R2;

    if isfinite(bestTraditionalARE) && bestTraditionalARE > 0
        seedResults.PGE_DFM_ARE_Improvement_vs_BestTraditional_percent(iSeed) = ...
            100 * (bestTraditionalARE - metPIETest.ARE) / bestTraditionalARE;

        seedResults.PGE_DFM_RCN_ARE_Improvement_vs_BestTraditional_percent(iSeed) = ...
            100 * (bestTraditionalARE - metPIEETest.ARE) / bestTraditionalARE;
    end
    if isfinite(metDirectTest.ARE) && metDirectTest.ARE > 0
        seedResults.PGE_DFM_ARE_Improvement_vs_DirectMLP_percent(iSeed) = ...
            100 * (metDirectTest.ARE - metPIETest.ARE) / metDirectTest.ARE;
    end
    if isfinite(metStaticTest.ARE) && metStaticTest.ARE > 0
        seedResults.PGE_DFM_ARE_Improvement_vs_StaticWeight_percent(iSeed) = ...
            100 * (metStaticTest.ARE - metPIETest.ARE) / metStaticTest.ARE;
    end

    fprintf('\n[Seed %d] TEST RESULTS\n', thisSeed);
    fprintf('PGE-DFM     : ARE=%.6f  MAE=%.6f  RMSE=%.6f  R2=%.6f\n', ...
        metPIETest.ARE, metPIETest.MAE, metPIETest.RMSE, metPIETest.R2);
    fprintf('PGE-DFM-RCN : ARE=%.6f  MAE=%.6f  RMSE=%.6f  R2=%.6f\n', ...
        metPIEETest.ARE, metPIEETest.MAE, metPIEETest.RMSE, metPIEETest.R2);
    fprintf('Direct-MLP  : ARE=%.6f  MAE=%.6f  RMSE=%.6f  R2=%.6f\n', ...
        metDirectTest.ARE, metDirectTest.MAE, metDirectTest.RMSE, metDirectTest.R2);
    fprintf('EqualWeight : ARE=%.6f  MAE=%.6f  RMSE=%.6f  R2=%.6f\n', ...
        metEqualTest.ARE, metEqualTest.MAE, metEqualTest.RMSE, metEqualTest.R2);
    fprintf('StaticWeight: ARE=%.6f  MAE=%.6f  RMSE=%.6f  R2=%.6f\n', ...
        metStaticTest.ARE, metStaticTest.MAE, metStaticTest.RMSE, metStaticTest.R2);
    ...
    if thisSeed == representativeSeed
        ...
        alphaTrainDirectRepresentative = alphaTrainDirect;
        alphaValDirectRepresentative = alphaValDirect;
        alphaTestDirectRepresentative = alphaTestDirect;
        metDirectTrainRepresentative = metDirectTrain;
        metDirectValRepresentative = metDirectVal;
        metDirectTestRepresentative = metDirectTest;
        metEqualTestRepresentative = metEqualTest;
        metStaticTestRepresentative = metStaticTest;
        wStaticTrainRepresentative = wStaticTrain;
    end
end
...
summaryModel = ["PGE-DFM"; "PGE-DFM-RCN"; "Direct-MLP"; "Equal-weight"; "Static-weight"];
summaryPrefix = {'PGE_DFM','PGE_DFM_RCN','DirectMLP','EqualWeight','StaticWeight'};
MultiSeed_Summary = table(summaryModel, repmat(nSeeds,numel(summaryPrefix),1), ...
    'VariableNames', {'Model','N_Seeds'});
for iMetric = 1:numel(metricNames)
    metricMean = NaN(numel(summaryPrefix),1);
    metricStd = NaN(numel(summaryPrefix),1);
    metricMin = NaN(numel(summaryPrefix),1);
    metricMax = NaN(numel(summaryPrefix),1);
    for iModelPrefix = 1:numel(summaryPrefix)
        valuesNow = seedResults.([summaryPrefix{iModelPrefix} '_Test_' metricNames{iMetric}]);
        metricMean(iModelPrefix) = mean(valuesNow, 'omitnan');
        metricStd(iModelPrefix) = std(valuesNow, 0, 'omitnan');
        metricMin(iModelPrefix) = min(valuesNow, [], 'omitnan');
        metricMax(iModelPrefix) = max(valuesNow, [], 'omitnan');
    end
    MultiSeed_Summary.(['Test_' metricNames{iMetric} '_Mean']) = metricMean;
    MultiSeed_Summary.(['Test_' metricNames{iMetric} '_Std']) = metricStd;
    MultiSeed_Summary.(['Test_' metricNames{iMetric} '_Min']) = metricMin;
    MultiSeed_Summary.(['Test_' metricNames{iMetric} '_Max']) = metricMax;
end
...
fprintf('\n参考基线：固定测试集上 ARE 最优传统模型 = %s, ARE = %.6f\n', ...
    char(bestTraditionalModelARE), bestTraditionalARE);
fprintf('PGE-DFM 相对 Direct-MLP 的 ARE 改善：均值 %.3f%%，范围 [%.3f%%, %.3f%%]\n', ...
    mean(seedResults.PGE_DFM_ARE_Improvement_vs_DirectMLP_percent,'omitnan'), ...
    min(seedResults.PGE_DFM_ARE_Improvement_vs_DirectMLP_percent,[],'omitnan'), ...
    max(seedResults.PGE_DFM_ARE_Improvement_vs_DirectMLP_percent,[],'omitnan'));
fprintf('PGE-DFM 相对固定训练均值权重的 ARE 改善：均值 %.3f%%，范围 [%.3f%%, %.3f%%]\n', ...
    mean(seedResults.PGE_DFM_ARE_Improvement_vs_StaticWeight_percent,'omitnan'), ...
    min(seedResults.PGE_DFM_ARE_Improvement_vs_StaticWeight_percent,[],'omitnan'), ...
    max(seedResults.PGE_DFM_ARE_Improvement_vs_StaticWeight_percent,[],'omitnan'));
...
multiSeedExcel = fullfile(multiSeedFolder, 'PGE_DFM_Reviewer_Experiments.xlsx');
...
MultiSeed_Config = table( ...
    2026, representativeSeed, nSeeds, string(mat2str(trainingSeeds)), tau, ...
    bestTraditionalModelARE, bestTraditionalARE, ...
    'VariableNames', { ...
    'FixedSplitSeed','RepresentativeTrainingSeed','NTrainingSeeds','TrainingSeeds','TauFromTrainingOnly', ...
    'BestTraditionalAREModel','BestTraditionalARE'} );

writetable(MultiSeed_Config, multiSeedExcel, 'Sheet', 'Config');

writetable(seedResults, fullfile(multiSeedFolder, 'PGE_DFM_Reviewer_PerSeed.csv'));
writetable(MultiSeed_Summary, fullfile(multiSeedFolder, 'PGE_DFM_Reviewer_Summary.csv'));

save(fullfile(multiSeedFolder, 'PGE_DFM_Reviewer_Experiments.mat'), ...
    'trainingSeeds','representativeSeed','seedResults','MultiSeed_Summary', ...
    'idxTrain','idxVal','idxTest','Fixed_Split_Table','MultiSeed_Config','tau');

fprintf('\n多随机种子结果已保存到：%s\n', multiSeedFolder);
fprintf('  - PGE_DFM_Reviewer_Experiments.xlsx\n');
fprintf('  - PGE_DFM_Reviewer_PerSeed.csv\n');
fprintf('  - PGE_DFM_Reviewer_Summary.csv\n');
fprintf('  - PGE_DFM_Reviewer_Experiments.mat\n');
...
alphaTrainDirect = alphaTrainDirectRepresentative;
alphaValDirect = alphaValDirectRepresentative;
alphaTestDirect = alphaTestDirectRepresentative;
metDirectTrain = metDirectTrainRepresentative;
metDirectVal = metDirectValRepresentative;
metDirectTest = metDirectTestRepresentative;
metEqualTest = metEqualTestRepresentative;
metStaticTest = metStaticTestRepresentative;
wStaticTrain = wStaticTrainRepresentative;
...
fprintf('PIE-DF-E Test: ARE=%.6f MAE=%.6f RMSE=%.6f R2=%.6f\n', ...
    metPIEETest.ARE, metPIEETest.MAE, metPIEETest.RMSE, metPIEETest.R2);
fprintf('Direct-MLP Test: ARE=%.6f MAE=%.6f RMSE=%.6f R2=%.6f\n', ...
    metDirectTest.ARE, metDirectTest.MAE, metDirectTest.RMSE, metDirectTest.R2);
...
forceUpdate13DBFigureMetrics_R2FINAL(figPIEDF_13DB_R2FINAL, 'PGE-DFM', outFolder, axisMin, axisMax);
...
% 同样强制修正 PIE-DF-E 子图
forceUpdate13DBFigureMetrics_R2FINAL(figPIEDFE_13DB_R2FINAL, 'PGE-DFM-RCN', outFolder, axisMin, axisMax);
...
legend( ...
    sprintf('PGE-DFM, MRE=%.3f', metPIE_all.ARE), ...
    sprintf('PGE-DFM-RCN, MRE=%.3f', metPIEE_all.ARE), ...
    'y = x', ...
    '\pm30%', ...
    'Location','northwest');
...
text(0.04, 0.96, ...
    sprintf('PGE-DFM: N=%d, MRE=%.3f, RMSE=%.3f', ...
...
text(0.04, 0.88, ...
    sprintf('PGE-DFM-RCN: N=%d, MRE=%.3f, RMSE=%.3f', ...
...
title('PGE-DFM expert-weight network training loss', ...
    'FontName','Times New Roman', ...
    'FontSize', 15, ...
    'FontWeight','normal');
...
title('PGE-DFM validation error history', ...
    'FontName','Times New Roman', ...
    'FontSize', 15, ...
    'FontWeight','normal');
...
title('PGE-DFM-RCN residual network training loss', ...
    'FontName','Times New Roman', ...
    'FontSize', 15, ...
    'FontWeight','normal');
...
title('PGE-DFM-RCN validation error history', ...
    'FontName','Times New Roman', ...
    'FontSize', 15, ...
    'FontWeight','normal');
...
title('Overall average expert weights of PGE-DFM model', ...
    'FontName','Times New Roman', ...
    'FontSize', 15, ...
    'FontWeight','normal');
...
%% ========================================================================
%  16B. Reviewer diagnostics: local weights, ablations, macro and bin errors
% ========================================================================

% Local variability is quantified pointwise and within each database.  A
% large within-database share rules out database identity as the sole source
% of variation.
weightStd = std(W_all_valid, 0, 1, 'omitnan');
weightEntropy = -sum(W_all_valid .* log(max(W_all_valid,1e-12)), 2) ./ log(M);
effectiveExperts = exp(-sum(W_all_valid .* log(max(W_all_valid,1e-12)), 2));
withinDatabaseVarianceShare = NaN(1,M);
for mm = 1:M
    wNow = W_all_valid(:,mm);
    totalSS = sum((wNow - mean(wNow,'omitnan')).^2, 'omitnan');
    withinSS = 0;
    for db = 1:max(groupID_valid)
        inDb = groupID_valid == db;
        if any(inDb)
            withinSS = withinSS + sum((wNow(inDb) - mean(wNow(inDb),'omitnan')).^2, 'omitnan');
        end
    end
    withinDatabaseVarianceShare(mm) = withinSS / max(totalSS, eps);
end

Weight_Local_Diagnostics = table(string(modelNames(:)), meanWeights(:), weightStd(:), ...
    withinDatabaseVarianceShare(:), ...
    'VariableNames', {'Model','MeanWeight','WeightSD','WithinDatabaseVarianceShare'});
writetable(Weight_Local_Diagnostics, ...
    fullfile(trainPlotFolder, 'PGE_DFM_Local_Weight_Diagnostics.csv'));

conditionValues = [P_MPa(validRows), G(validRows), j_g(validRows), ...
                   pitchRatio(validRows), Y_valid];
conditionNames = {'Pressure_MPa','MassFlux','GasSuperficialVelocity', ...
                  'PitchToDiameter','MeasuredVoidFraction'};
Weight_Condition_Correlations = table;
for mm = 1:M
    for cc = 1:numel(conditionNames)
        validCorr = isfinite(W_all_valid(:,mm)) & isfinite(conditionValues(:,cc));
        if sum(validCorr) >= 3
            R = corrcoef(W_all_valid(validCorr,mm), conditionValues(validCorr,cc));
            corrNow = R(1,2);
        else
            corrNow = NaN;
        end
        newRow = table(string(modelNames{mm}), string(conditionNames{cc}), corrNow, ...
            'VariableNames', {'Model','Condition','PearsonR'});
        Weight_Condition_Correlations = [Weight_Condition_Correlations; newRow]; %#ok<AGROW>
    end
end
writetable(Weight_Condition_Correlations, ...
    fullfile(trainPlotFolder, 'PGE_DFM_Weight_Condition_Correlations.csv'));

% Figure requested by the reviewer: the three most variable expert weights
% against pressure, mass flux, gas velocity, geometry, and measured alpha.
[~, mostVariableExperts] = sort(weightStd, 'descend');
mostVariableExperts = mostVariableExperts(1:min(3,M));
figLocal = figure('Color','w', 'Position',[50 50 1500 820]);
tiledlayout(numel(mostVariableExperts), numel(conditionNames), ...
    'TileSpacing','compact', 'Padding','compact');
for rr = 1:numel(mostVariableExperts)
    mm = mostVariableExperts(rr);
    for cc = 1:numel(conditionNames)
        nexttile;
        scatter(conditionValues(:,cc), W_all_valid(:,mm), 12, groupID_valid, 'filled');
        box on;
        if rr == numel(mostVariableExperts), xlabel(conditionNames{cc}, 'Interpreter','none'); end
        if cc == 1, ylabel(sprintf('%s weight', modelNames{mm}), 'Interpreter','none'); end
        set(gca,'FontName','Times New Roman','FontSize',9);
    end
end
exportgraphics(figLocal, fullfile(trainPlotFolder, ...
    'PGE_DFM_Local_Weights_vs_Conditions.png'), 'Resolution', 300);
savefig(figLocal, fullfile(trainPlotFolder, 'PGE_DFM_Local_Weights_vs_Conditions.fig'));

Weight_Distribution_Summary = table( ...
    mean(weightEntropy,'omitnan'), std(weightEntropy,0,'omitnan'), ...
    mean(effectiveExperts,'omitnan'), min(effectiveExperts,[],'omitnan'), ...
    max(effectiveExperts,[],'omitnan'), mean(withinDatabaseVarianceShare,'omitnan'), ...
    'VariableNames', {'NormalizedEntropyMean','NormalizedEntropySD', ...
    'EffectiveExpertsMean','EffectiveExpertsMin','EffectiveExpertsMax', ...
    'MeanWithinDatabaseVarianceShare'});
writetable(Weight_Distribution_Summary, ...
    fullfile(trainPlotFolder, 'PGE_DFM_Weight_Distribution_Summary.csv'));

nParamExpert = countNetworkParametersPIEOneClick(netExpert);
nParamResidual = countNetworkParametersPIEOneClick(netErr);
nParamDirect = countNetworkParametersPIEOneClick(netDirect);
Reviewer_Ablation = table( ...
    ["Best conventional DFM"; "Equal-weight experts"; "Static training-mean weights"; ...
     "Direct-MLP"; "PGE-DFM"; "PGE-DFM-RCN"], ...
    [0; 0; 0; nParamDirect; nParamExpert; nParamExpert+nParamResidual], ...
    [bestTraditionalARE; metEqualTest.ARE; metStaticTest.ARE; metDirectTest.ARE; ...
     metPIETest.ARE; metPIEETest.ARE], ...
    [singleMetrics.MAE(bestTraditionalIdx); metEqualTest.MAE; metStaticTest.MAE; ...
     metDirectTest.MAE; metPIETest.MAE; metPIEETest.MAE], ...
    [singleMetrics.RMSE(bestTraditionalIdx); metEqualTest.RMSE; metStaticTest.RMSE; ...
     metDirectTest.RMSE; metPIETest.RMSE; metPIEETest.RMSE], ...
    [singleMetrics.R2(bestTraditionalIdx); metEqualTest.R2; metStaticTest.R2; ...
     metDirectTest.R2; metPIETest.R2; metPIEETest.R2], ...
    'VariableNames', {'Model','TrainableParameters','TestMRE','TestMAE','TestRMSE','TestR2'});
writetable(Reviewer_Ablation, fullfile(trainPlotFolder, ...
    'Reviewer_Ablation_RepresentativeSeed.csv'));

% Database-wise results and unweighted database macro averages.
Reviewer_Database_Metrics = table;
for db = 1:max(groupID_valid)
    localTest = idxTest(groupID_valid(idxTest) == db);
    if isempty(localTest), continue; end
    predSet = {alphaTestPIE(groupID_valid(idxTest)==db), ...
               alphaTestPIEE(groupID_valid(idxTest)==db), ...
               alphaTestDirect(groupID_valid(idxTest)==db)};
    modelSet = {'PGE-DFM','PGE-DFM-RCN','Direct-MLP'};
    for kk = 1:numel(modelSet)
        metNow = calcMetricsPIEOneClick_R2FIX20260513(Y_valid(localTest), predSet{kk});
        Reviewer_Database_Metrics = [Reviewer_Database_Metrics; ...
            table(db, string(modelSet{kk}), numel(localTest), metNow.ARE, metNow.MAE, ...
            metNow.RMSE, metNow.R2, 'VariableNames', ...
            {'DatabaseID','Model','N','MRE','MAE','RMSE','R2'})]; %#ok<AGROW>
    end
end
writetable(Reviewer_Database_Metrics, fullfile(trainPlotFolder, ...
    'Reviewer_Database_Wise_Test_Metrics.csv'));
Reviewer_Database_Macro = groupsummary(Reviewer_Database_Metrics, 'Model', 'mean', ...
    {'MRE','MAE','RMSE','R2'});
writetable(Reviewer_Database_Macro, fullfile(trainPlotFolder, ...
    'Reviewer_Database_Macro_Test_Metrics.csv'));

% Low-alpha behavior is reported using both relative and absolute errors.
alphaBinEdges = [0 0.1 0.2 0.4 0.6 0.8 1.000001];
alphaBinLabels = ["[0,0.1)"; "[0.1,0.2)"; "[0.2,0.4)"; ...
                  "[0.4,0.6)"; "[0.6,0.8)"; "[0.8,1]"];
Reviewer_AlphaBin_Metrics = table;
testTruth = Y_valid(idxTest);
testPredictions = {alphaTestPIE, alphaTestPIEE, alphaTestDirect};
testModelNames = {'PGE-DFM','PGE-DFM-RCN','Direct-MLP'};
for bb = 1:numel(alphaBinLabels)
    inBin = testTruth >= alphaBinEdges(bb) & testTruth < alphaBinEdges(bb+1);
    for kk = 1:numel(testModelNames)
        if any(inBin)
            metNow = calcMetricsPIEOneClick_R2FIX20260513(testTruth(inBin), testPredictions{kk}(inBin));
        else
            metNow = struct('ARE',NaN,'MAE',NaN,'RMSE',NaN,'R2',NaN);
        end
        Reviewer_AlphaBin_Metrics = [Reviewer_AlphaBin_Metrics; ...
            table(alphaBinLabels(bb), string(testModelNames{kk}), sum(inBin), ...
            metNow.ARE, metNow.MAE, metNow.RMSE, metNow.R2, ...
            'VariableNames', {'AlphaBin','Model','N','MRE','MAE','RMSE','R2'})]; %#ok<AGROW>
    end
end
writetable(Reviewer_AlphaBin_Metrics, fullfile(trainPlotFolder, ...
    'Reviewer_Alpha_Bin_Test_Metrics.csv'));
%% ========================================================================
%  17. 重新保存，包含训练曲线和权重表
% ========================================================================
...
save(fullfile(outFolder, 'PIE_DF_OneClick_Model_And_13DB_Predictions.mat'), ...
    ...
    'Weight_Local_Diagnostics', ...
    'Weight_Condition_Correlations', ...
    'Weight_Distribution_Summary', ...
    'Reviewer_Ablation', ...
    'Reviewer_Database_Metrics', ...
    'Reviewer_Database_Macro', ...
    'Reviewer_AlphaBin_Metrics', ...
    'tau');
fprintf('\n训练过程图和权重图已保存到文件夹：\n%s\n', trainPlotFolder);
...
function alpha0 = initialAlphaFromFluxPIEOneClick(jg, jt)
    % Target-independent homogeneous starting value for implicit DFM solves.
    % Only predictor-side superficial velocities are used.
    if ~isfinite(jg) || ~isfinite(jt) || jt <= 0
        alpha0 = 0.2;
    else
        alpha0 = jg / max(jt, eps);
        alpha0 = min(max(alpha0, 0.01), 0.99);
    end
end

function [loss, gradients] = directGradientsPIEOneClick(net, dlX, dlY)
    alphaHat = forward(net, dlX);
    loss = mean((alphaHat - dlY).^2, 'all');
    gradients = dlgradient(loss, net.Learnables);
end

function alphaHat = predictDirectPIEOneClick(net, X)
    dlX = dlarray(single(X'), 'CB');
    dlAlpha = predict(net, dlX);
    alphaHat = gather(extractdata(dlAlpha))';
    alphaHat = clipAlphaPIEOneClick(alphaHat);
end

function n = countNetworkParametersPIEOneClick(net)
    n = 0;
    for ii = 1:height(net.Learnables)
        n = n + numel(net.Learnables.Value{ii});
    end
end
